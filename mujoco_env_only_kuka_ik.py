...
class KukaTennisEnv(gym.Env):
    def __init__(self,proc_id=0,history=4):
        ...
        self.action_space = spaces.Box(low=-0.03, high=0.03, shape=(7,), dtype=np.float32)  # Adjust based on your actuator count
        self.observation_space = spaces.Box(low=-np.inf, high=np.inf, shape=(self.model.nq + 7 + 7,), dtype=np.float32)
    ...
    def restrict_joint_angles(self,joint_info):
        joint_info[0] = np.clip(joint_info[0],-2.9,2.9)
        joint_info[1] = np.clip(joint_info[1],-2.0,2.0)
        joint_info[2] = np.clip(joint_info[2],-2.9,2.9)
        joint_info[3] = np.clip(joint_info[3],-2.0,2.0)
        joint_info[4] = np.clip(joint_info[4],-2.9,2.9)
        joint_info[5] = np.clip(joint_info[5],-2.0,2.0)
        joint_info[6] = np.clip(joint_info[6],-3.0,3.0)
        return joint_info
    ...
    def step(self, action):
        ...
        # print(self.curr_qpos,self.data.qpos[:7])
        self.curr_qpos = np.array(action) + np.array(self.curr_qpos)
        # self.data.qpos[1:] = 0.
        self.curr_qpos = self.restrict_joint_angles(self.curr_qpos)
        self.data.qpos[:7] = np.array(self.curr_qpos).copy()
        self.data.qvel[:7] = 0. 
        ...
        obs = np.float32(np.concatenate([self.data.qpos,diff_pos,diff_quat,self.curr_target]))
        # print(self.data.qpos)
        # Calculate reward and done
        reward = self._calculate_reward()
        curr_reward = reward - self.prev_reward - 150*np.sum(np.abs(self.prev_actions[-1,:]))/(7000*0.03)
    ...
    def reset_pose(self, pose):
        self.curr_qpos = np.array(pose).copy()
        self.data.qpos[:7] = self.curr_qpos.copy()
        mj.mj_step(self.model, self.data)
    ...
    def reset(self,seed=None):
        ...
        self.curr_qpos = np.zeros(7)
        for i in range(7):
            self.curr_qpos[i] = np.random.uniform(-1.,1.)*0.5
        self.data.qpos[:7] = np.array(self.curr_qpos).copy()
