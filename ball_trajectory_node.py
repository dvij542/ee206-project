...
class BallTracker:
    def __init__(self):
        ...
        rospy.Subscriber('/ball_info', Twist, self.ball_callback)
        self.target_pub = rospy.Publisher('/predicted_pose', Pose, queue_size=10)
    ...
    def calc_target_racket_pose(self, ball_pos, ball_vel, bounce_factor=1, table_z=TABLE_HEIGHT):
        ...
        # Publish curr_target set by set_target_pose to /predicted_pose
        pose_msg = Pose()
        pose_msg.position.x = self.curr_target[0]
        pose_msg.position.y = self.curr_target[1]
        pose_msg.position.z = self.curr_target[2]
        pose_msg.orientation.x = self.curr_target[3]
        pose_msg.orientation.y = self.curr_target[4]
        pose_msg.orientation.z = self.curr_target[5]
        pose_msg.orientation.w = self.curr_target[6]
